...
import numpy as np
from . import utils
from .surface import Plane,HalfSphere,Parabola
...
class ConcaveMirror(Optics):
    ...
    def __init__(self,topsurface,position=None,normal=None,boundaryradius=2.0):
        if position is None:
            position = [0,0,0]
        if normal is None:
            normal = [0,0,1]
        ...
        self.boundaryradius = boundaryradius  # circular boundary radius
        # Surface needs to be a halfsphere or parabola
        if isinstance(topsurface,HalfSphere)==False and isinstance(topsurface,Parabola)==False:
            raise ValueError('topsurface must be HalfSphere or Parabola')

        # "position" is always at the Vertex of the topsurface
        # the "depth" is determined by the topsurface shape and the boundary radius

        # there is potential for there to be inconsistencies between the
        # topsurface.position/normal and the input position/normal
        # maybe we don't need any input position/normal
        # we could require that from the topsurface
        
    def __repr__(self):
        dd = (*self.position.data,*self.normal.data,self.boundaryradius)
        s = 'ConcaveMirror(o=[{:.3f},{:.3f},{:.3f}],n=[{:.3f},{:.3f},{:.3f}],r={:.3f}])'.format(*dd)
        return s
    ...
    def distance(self,points):
        """ Return distance to mirror """
        return self.position.distance(points)
    ...
    def ison(self,points):
        ...
        for i in range(npts):
            ...
            isonsurface = self.topsurface.ison(data[i])
            if isonsurface==False:
                isonmirror[i] = False
                continue

            import pdb; pdb.set_trace()
            # Check if it is inside the circular boundary
            #  rotate the points into the frame of the plane so it is a 2D problem
            if self.vertices is None:
                isonmirror[i] = True
                continue
    ...
    def reflection(self,ray,point):
        ...
        # First we have to reverse the lightray's normal
        # and then reflect it about the mirror's normal
        mirror_normal = self.topsurface.normalatpoint(point)
        ray_normal = ray.normal.copy()
        # Flip the ray's normal, since it will be going "out"
        #   after it is reflected
        ray_flip_normal = -1*ray_normal
